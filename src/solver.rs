...
impl Solver {
    pub fn create(&mut self, arguments: std::vec::Vec<String>) -> Result<(), ClingoServerError> {
        match self {
            Solver::SolveHandle(_) => Err(ClingoServerError::InternalError {
                msg: "Solver still running!",
            }),
            Solver::Control(_) => {
    ...
    pub fn close(&mut self) -> Result<(), ClingoServerError> {
        match self {
            Solver::Control(_) => Err(ClingoServerError::InternalError {
                msg: "Solving has not yet started",
            }),
            Solver::SolveHandle(handle) => match handle.take() {
                None => Err(ClingoServerError::InternalError {
                    msg: "No SolveHandle",
                }),
                Some(handle) => {
    ...
    pub fn solve(
        ...
        mode: SolveMode,
        assumptions: &[Literal],
    ) -> Result<(), ClingoServerError> {
        match self {
            Solver::SolveHandle(_) => Err(ClingoServerError::InternalError {
                msg: "Solving has already started",
            }),
    ...
    pub fn add(
        ...
        match self {
            Solver::SolveHandle(_) => Err(ClingoServerError::InternalError {
                msg: "add Solver has been already started",
            }),
            Solver::Control(ctl) => match ctl.as_mut() {
                None => Err(ClingoServerError::InternalError {
                    msg: "No Control object",
                }),
                Some(ctl) => {
    ...
    pub fn ground(&mut self, parts: &[Part]) -> Result<(), ClingoServerError> {
        match self {
            Solver::SolveHandle(_) => Err(ClingoServerError::InternalError {
                msg: "ground Solver has been already started",
            }),
            Solver::Control(ctl) => match ctl.as_mut() {
                None => Err(ClingoServerError::InternalError {
                    msg: "No Control object",
    ...
    pub fn model(&mut self) -> Result<Option<&Model>, ClingoServerError> {
        match self {
            Solver::Control(_) => Err(ClingoServerError::InternalError {
                msg: "Solving has not yet started",
            }),
    ...
    pub fn resume(&mut self) -> Result<(), ClingoServerError> {
        match self {
            Solver::Control(_) => Err(ClingoServerError::InternalError {
                msg: "resume: Solver has not yet started",
            }),
            Solver::SolveHandle(handle) => match handle.as_mut() {
                None => Err(ClingoServerError::InternalError {
                    msg: "No SolveHandle",
